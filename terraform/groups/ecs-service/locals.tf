...
locals {
  ...
  service_name                = "document-store-delta-consumer"
  container_port              = "8080"
  docker_repo                 = "document-store-delta-consumer"
  kms_alias                   = "alias/${var.aws_profile}/environment-services-kms"
  healthcheck_path            = "/document-store-delta-consumer/healthcheck"
  healthcheck_matcher         = "200"
  vpc_name                    = local.stack_secrets["vpc_name"]
  s3_config_bucket            = data.vault_generic_secret.shared_s3.data["config_bucket_name"]
  app_environment_filename    = "document-store-delta-consumer.env"
